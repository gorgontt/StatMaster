...
class ScoreCalculatorTest {
    ...
    @Test
    fun `calculateScore handles empty questions list`() {
        val questions = emptyList<QuestionWithAnswers>()
        val userAnswers = emptyMap<Int, Int?>()

        val (correct, total) = calculateScore(questions, userAnswers)

        assertEquals("No correct answers", 0, correct)
        assertEquals("No questions", 0, total)
    }
