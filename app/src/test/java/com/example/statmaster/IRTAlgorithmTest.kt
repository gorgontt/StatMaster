...
class IrtAlgorithmTest {
    ...
    fun `correct answer increases ability`() {
        ...
        assertTrue("Ability should not change too much", abs(result - currentAbility) < 0.5f)
    }
    ...
    @Test
    fun `ability changes within reasonable bounds`() {
        val currentAbility = 2.0f
        val difficultyValue = 1.0f
        ...
        assertTrue("Ability should be between 1.5 and 2.5", result in 1.5f..2.5f)
    }
