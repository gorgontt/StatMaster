...
class LevelStatRepository(private val authManager: AuthManager,  private val context: Context ) {

    suspend fun getAllLevels(): List<Level> {
        return try {
            authManager.supabase.postgrest["level_stat"]
                .select {
                    order("order_number", Order.ASCENDING)
                }
                .decodeList<Level>()
    ...
    suspend fun getLevelDocument(levelId: Int): LevelDocument? {
        return try {
            authManager.supabase.postgrest
                .from("levels_documents_stat?select=*&level_id=eq.$levelId")
                .select()
                .decodeSingleOrNull<LevelDocument>()
        } catch (e: Exception) {
            Log.e("Supabase", "Error loading document", e)
    ...
    suspend fun getTestByLevelId(levelId: Int): Test? {
        return try {
            ...
            // Получаем тест
            authManager.supabase.postgrest
                .from("test_stat?select=id,level_id,title,description&level_id=eq.$levelId")
                .select()
                .decodeSingleOrNull<Test>()
                .also { test ->
                    if (test == null) {
    ...
    suspend fun debugAllTests(): List<Test> {
        return try {
            authManager.supabase.postgrest
                .from("test_stat?select=id,level_id,title,description")
                .select()
                .decodeList<Test>()
                .also { tests ->
                    Log.d("Supabase", "All tests in DB: ${tests.joinToString()}")
    ...
    suspend fun getQuestionsWithAnswers(testId: Int): List<QuestionWithAnswers> {
        return try {
            // Получаем вопросы для теста
            val questions = authManager.supabase.postgrest
                .from("question_stat?select=*&test_id=eq.$testId&order=order_number.asc")
                .select()
                .decodeList<QuestionWithAnswers>()
                .also {
                    Log.d("Supabase", "Loaded ${it.size} questions for test $testId")
            ...
            questions.map { question ->
                val answers = authManager.supabase.postgrest
                    .from("answer_stat?select=*&question_id=eq.${question.id}&order=order_number.asc")
                    .select()
                    .decodeList<Answer>()
                    .also {
                        Log.d("Supabase", "Loaded ${it.size} answers for question ${question.id}")
    ...
    suspend fun updateLevelCompletion(levelId: Int, isCompleted: Boolean): Boolean {
        return try {
            Log.d("LevelStatRepository", "Updating level $levelId to completed=$isCompleted")
            ...
            // 3. Возвращаем успех без проверки (временно)
            Log.d("LevelStatRepository", "Level $levelId update request completed")
            true
        } catch (e: Exception) {
            Log.e("LevelStatRepository", "Error updating level", e)
            false
        }
    }
    ...
    suspend fun getLevel(levelId: Int): Level? {
        return try {
            ...
            Log.e("LevelStatRepository", "Error getting level", e)
            null
        }
    }
    ...
    suspend fun getLevelById(levelId: Int): Level? {
        return try {
            // Проверяем сначала локальное хранилище
            val sharedPref = context.getSharedPreferences("LevelProgress", Context.MODE_PRIVATE)
            val isCompletedLocally = sharedPref.getBoolean("level_stat_$levelId", false)
    ...
    suspend fun getTestResults(levelId: Int): Pair<Int, Int>? {
        return try {
            ...
            val sharedPref = context.getSharedPreferences("TestAnswers", Context.MODE_PRIVATE)
            val userAnswers = questions.associate { q ->
                q.id to sharedPref.getInt("answer_stat_${levelId}_${q.id}", -1)
            }
